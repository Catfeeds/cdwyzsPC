...
class index extends foreground {
	...
	public function init() {
		...
		//初始化phpsso
		$phpsso_api_url =  $r = $this->db->get_one(array('userid'=>intval($_userid))); if($r[username]!=$_username){ return '2'; } $this->_init_phpsso(); 
		//获取头像数组
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			is_password($_POST['password']) && is_badword($_POST['password'])==false ?  addslashes(urldecode(trim($_POST['password']))) :  showmessage(L('password_format_incorrect'), HTTP_REFERER);
