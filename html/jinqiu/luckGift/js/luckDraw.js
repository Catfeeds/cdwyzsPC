...
function getdrid (){
	...
	$(function(){
	window.setInterval("getdrid()", 10000);
	var clickEvent=$("#getValue").text();
        ...
        init: function() {
			...
			if(length < allNumber){
				for(var i=length;i<=(allNumber-length);i++){
					$this.append("<li>"+(i+1)+"<img src=./images/"+(i+1)+".png width='50px' height='50px' />"+"</li>");
				}
			}
			...
			*/
			...
			$(click).on('click',function(){
				if(!flg){
					flg = true;
					target = Math.floor(Math.random()*allNumber+1);
					console.log("*****"+target);
					speedUp();
				}else{
					return ;
